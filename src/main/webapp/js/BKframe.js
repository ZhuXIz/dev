	...
	$.fn.BKframe = function(options) {
		...
		return this.each(function() {
			...
			function currentTime() {
				...
				str += '<span class="day">' + d.getDate() + '日</span>-';
				if(d.getDay()==0){
					str += '<span>' + weekday[6] + '</span></div>';
				}else{
			...
			$.learuntab = {
				...
				skinchange: function(skinName) {
					$("#skin").attr("href", "/wx/skin/" + skinName + "/skin.css");
			...
			function creatIframe(href, titleName,id, pageid) {
				...
				showBox.find('iframe').attr("src", href).on('load',function(){
