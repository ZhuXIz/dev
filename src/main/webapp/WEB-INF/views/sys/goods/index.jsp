	           ...
	           function selectGoods(){
	        	 ...
	        	 console.log($('#table'));
           		...
           		<div style="text-align: center; ">
	           		 <input  type="text" placeholder="通过商品标题进行关键字查询" id="keyWords" >
	           		 <button id="search" onclick="javascript:selectGoods();" >查询</button>
           		</div>
...
<div class="modal fade" id="addGoodsModel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div style="height: 30px;width: 100%;background-color: gray; text-align: center;"> <p>商品信息</p></div><br/>
					<div class="row">
						<div class="col-lg-10">
	...
	function setGoods_type(obj,type,genreName){
		...
		$('#goodsType').val(type);
					...
					<!-------------------------------------------------------------- 所属商家---------------------------------------------- -->
					<div style="height: 30px;width: 100%;background-color: gray; text-align: center;"> <p>所属商家</p></div><br/>
					...
					<div class="row">
							<div class="col-lg-10">
								<div class="input-group">
									...
									<img src="${pageContext.request.contextPath}/upload/imgs/upload.png" onclick="$('#gUpload').click();"><br/><br/>
									<img  src="" id="gIntroduceImageShow" style="max-width: 50px">
									<input type="file" name="logoFile" id="gUpload" onchange="setGImg(this)" style="display: none;">
									<input type="hidden" id="gIntroduceImage" class="form-control">
								</div>
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-lg-10">
							<div class="input-group">
								联系人:
								<input type="text" class="form-control" id="gPhoneName">
							</div>
						</div>
					</div>
					<br/>
					...
					<div class="row">
						<div class="col-lg-10">
							<div class="input-group">
								联系电话:
								<input type="text" class="form-control" id="gPhoneNum">
							</div>
						</div>
					</div>
					<br/>
					...
					<div class="row">
						<div class="col-lg-10">
							<div class="input-group">
								地址:
								<input type="text" class="form-control" id="gAddressStr">
							</div>
						</div>
					</div>
					<br/>
					...
					<div class="row">
						<div class="col-lg-10">
							<div class="input-group">
								营业时间:
								<input type="text" class="form-control" id="gOnTime">
							</div>
						</div>
					</div>
					<br/>
					...
					<div class="row">
						<div class="col-lg-10">
							<div class="input-group">
								地图:
								<div id="gContainer" >
									<input type="hidden" class="form-control" id="gAddress">
								</div>
					...
					<div class="row">
						<div class="col-lg-10">
							<div class="input-group">
							商家介绍:
							<textarea rows="8" cols="40" id="gIntroduce" placeholder="商家介绍（可不填）"  ></textarea>
							</div>
						</div>
					</div>
					<br/>
...
function setGMap(add){
	    ...
	    map = new qq.maps.Map(document.getElementById('gContainer'),{
	        center: center,
	        zoom: 15
	    });
...
function initSeller(id){
 ...
 function checkSeller(){
	...
	if(introduceText.length<20){
		alert("详细介绍应不少于20个字");
		return false;
	}
	if(introduceText.length>500){
		alert("详细介绍应不多于500个字");
		return false;
	}
...
function removeImg(obj,index){
	...
	if(message){
		...
		console.log("imageStr:"+imageStr+"index:"+index);
		alert("删除成功!");
		return true;
	}else{
		alert("取消删除!");
...
function initGoods(id){
    ...
    success: function(ret) {
    	...
    	$.each(ret,function(index,obj){
    		type_html=type_html+'<li><a onclick="setGoods_type(this,'+obj.id+',\''+obj.genreName+'\')">'+obj.genreName+'</a></li>';
    	})
		...
		$.ajax({
			...
			success : function(msg) {
				...
				if(msg){
					...
					//
					if(msg.goodsType==null){
						$('.goodsTypeDrop').html('商品类型<b class="caret"></b>');
						$("#goodsType").val(null);
					...
					//商品类型
					if(msg.payType==null){
						$('.payTypeDrop').html('支付类型<b class="caret"></b>');
						$("#payType").val(null);
					}else{
					...
					//商家
					if(msg.seller!=null){
						currentSellerId=msg.seller.id;
						$("#gName").val(msg.seller.name);
						$("#gOnTime").val(msg.seller.onTime);
						$("#gPhoneNum").val(msg.seller.phoneNum);
						//$("#address").val(msg.seller.address);
						setGMap(msg.seller.address);
						$("#gIntroduce").val(msg.seller.introduceText);
						$("#gIntroduceImage").val(msg.seller.introduceImage);
						$("#gIntroduceImageShow").attr("src",path_pre+msg.seller.introduceImage);
						$("#gPhoneName").val(msg.seller.phoneName);
						$("#gAddressStr").val(msg.seller.addressStr);
					}else{
						initGoodsView();
					}
...
function initGoodsView(){
 ...
 function checkGoods(){
		...
		//商品验证
		if(typeof(titleName)=="undefined"||titleName.trim().length<=0){	
			alert("请填写商品标题！");
			return false;
		} 
		//var content = $(".imageShowCon").html();
		if(imageStr.length<=0){
			alert("请添加至少一张轮播图！");
			return false;
		}
		
		if(typeof(titleImage)=="undefined"||titleImage.trim().length<=0){	
			alert("请添加商品图！");
			return false;
		} 
		...
		if(typeof(resNum)=="undefined"||resNum.trim().length<=0){	
			alert("请添加优惠券数量！");
			return false;
		} 
		if(typeof(payType)=="undefined"||payType.trim().length<=0||payType==null){	
			alert("请添加支付类型！");
			return false;
		} 
		if(typeof(goodsType)=="undefined"||goodsType.trim().length<=0||goodsType==null){	
			alert("请添加商品类型！");
			return false;
		} 
		if (!/^\d+$/.test(resNum)) {
			alert("优惠券应为数字,且为非负整数！无限制请填0");	
			return false;
		}
		if (!/^\d+$/.test(browNum)) {
			alert("浏览量应为非负整数");
	...
	var gName = $("#gName").val();
	var gOnTime = $("#gOnTime").val();
	var gPhoneNum = $("#gPhoneNum").val();
	var gAddress = commonMarker.position+"";
	$("#gAddress").val(gAddress);
	//console.log($("#address").val());
	var gIntroduce = $("#gIntroduce").val();
	var gIntroduceImage = $("#gIntroduceImage").val();
	var gPhoneName = $("#gPhoneName").val();
	var gAddressStr = $("#gAddressStr").val();
	
  	//验证
	if(typeof(gName)=="undefined"||gName.trim().length<=0){	
		alert("请填写商家名！");
		return false;
	} 
	if(typeof(gOnTime)=="undefined"||gOnTime.trim().length<=0){	
		alert("请填写营业时间");
		return false;
	} 
	var pattern = /^((0\d{2,3}-?\d{7,8})|(1[35784]\d{9}))$/;
	var result =pattern.test(gPhoneNum);
	if(gPhoneNum && !result){
		 alert("不支持的手机格式"); 
		 return false;
	} 
	if(typeof(gPhoneNum)=="undefined"||gPhoneNum.trim().length<=0){	
		alert("请填写联系电话！");
		return false;
	} 
	/* if(typeof(address)=="undefined"||address.trim().length<=0){	
	...
	if(typeof(gIntroduceImage)=="undefined"||gIntroduceImage.trim().length<=0){	
		alert("请上传店铺图片！");
		return false;
	} 
	
	if(typeof(gPhoneName)=="undefined"||gPhoneName.trim().length<=0){	
		alert("请填写联系人！");
		return false;
	} 
	if(typeof(gAddressStr)=="undefined"||gAddressStr.trim().length<=0){	
		alert("请填写详细地址！");
		return false;
	} 
	
	if(gIntroduce.length<20){
		alert("详细介绍应不少于20个字");
		return false;
	}
	if(gIntroduce.length>500){
		alert("详细介绍应不多于500个字");
		return false;
	}
	...
	//alert(address);
	 var data ={
		...
		'seller':{
			...
			'phoneNum':gPhoneNum,
			'address':gAddress,
			'introduceText':gIntroduce,
			'introduceImage':gIntroduceImage,
			'phoneName':gPhoneName,
			'addressStr':gAddressStr
		}
	};
	saveGoods(data);
 }
		 ...
		 columns : [/* {                         
             ...
             align : 'center',
	            formatter: function(value, row, index) {
	            	...
	            	/* 
	            	console.log("type:"+param_Map.goodsId);

	            	var ss = '{"goodsId":4,"sellerId":4,"Id":null}';
	            	var json = JSON.parse(ss);
	           		console.log("ssssss"+json.goodsId); */
	            	//
	            	//处理格式化数据
            		return '<button class="btn button_btn bg-deep-blue" data-toggle="modal" onclick="javascript:initSpellList('+row.id+',\''+sellerId+'\','+value+')">'+
	            	'<input id="goodsItem" type="hidden" value="'+value+'">'+btnText+'</button>';
 	        }
         },{
             field : 'goodsType.genreName',
             title : '商品类型',
             align : 'center'
         },{
             field : 'payType',
	...
	function setGImg(obj){
	    ...
	    $.ajax({
	        ...
	        success: function(ret) {
	        	console.log(ret);
	            if(ret.code==0){
	                    $("#gIntroduceImage").val(ret.result.url);//将地址存储好
	                    $("#gIntroduceImageShow").attr("src",path_pre+ret.result.url);//显示图片
	                    alert(ret.message);
	            }else{
	                alertError(ret.message);
	                $("#gIntroduceImage").val("");
