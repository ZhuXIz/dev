        ...
        success: function(ret) {
            ...
            if(ret.code==0){
            	...
            	// imageStr+="&"+ret.result.url;
                 ...
                 var showHtml ="<img src='/wx"+ret.result.url+"' class='autoImg'/><input value='"+ret.result.url+"' type='hidden' name='image' id='goodsImage'/>";
               	//显示图片  
...
function setImg(obj){
    ...
    $.ajax({
        ...
        success: function(ret) {
        	console.log(ret);
            if(ret.code==0){
            		$('.titleImage').html("<span><img id='photourlShow' class='autoImg'/></span>");
...
function checkGoods(){
	...
	var price = $("#price").val();
	var goodsType2 = $("#goodsType").val();
	var payType2 = $("#payType").val();
	...
	if(payType2.length<=0){
		alert("请填写支付类型");
		return false;
	}
	if(goodsType2.length<=0){
		alert("请填写商品类型");
		return false;
	}
